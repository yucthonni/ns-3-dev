...
NrHelper::DoAssignStreamsToChannelObjects(Ptr<NrSpectrumPhy> phy, int64_t currentStream)
{
    ...
    if (pl)
    {
        ...
        {
            currentStream += pl->AssignStreams(currentStream);
    ...
    {
        // No propagation loss model added; skip this section.
    }

    // Now propagationLossModel variable exists in this scope. use it only if it is not 0.
    if (propagationLossModel)
    {
