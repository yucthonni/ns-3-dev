...
QdChannelModel::GetTypeId(void)
{
    static TypeId tid =
        TypeId("ns3::QdChannelModel")
            ...
            .AddAttribute("Path",
                          "Folder path containing the QD input files",
                          StringValue("TestFolder"),
                          MakeStringAccessor(&QdChannelModel::m_path),
                          MakeStringChecker())
            .AddAttribute(
                "Scenario",
                "Folder name for the QD scenario (contains Input/ and Output/ directories)",
                StringValue("Custom"),
                MakeStringAccessor(&QdChannelModel::m_scenario),
                MakeStringChecker())
            .AddAttribute(
                "Frequency",
...
QdChannelModel::SetPath(std::string path)
{
    ...
    TrimFolderName(path);
    m_path = path;
    // m_path = "contrib/nr/utils/channels/qd/QD/";
}
    ...
    Ptr<MatrixBasedChannelModel::ChannelMatrix> channelMatrix) const
{
    ...
    // if (update)
    // {
    //     std::cout << "Timestamp " << Simulator::Now().GetMilliSeconds() << std::endl;
    //     std::cout << "ChannelMatrix is getting updated" << std::endl;
    // }
                           ...
                           Ptr<const PhasedArrayModel> bAntenna)
{
    ...
    uint64_t channelId = GetKey(aId, bId);
                              ...
                              Ptr<const PhasedArrayModel> bAntenna)
{
    ...
    for (uint64_t mpcIndex = 0; mpcIndex < qdInfo.numMpcs; ++mpcIndex)
    {
        ...
        double pathGain = qdInfo.pathGain_dbpow[mpcIndex]; // TR++ Fix Paolo
        ...
        for (uint64_t bIndex = 0; bIndex < bSize; ++bIndex)
        {
            ...
            for (uint64_t aIndex = 0; aIndex < aSize; ++aIndex)
            {
                ...
                // Problem here 0 is not the correct index
    ...
    // std::cout << "qdInfo.azAod_deg: " << qdInfo.azAod_rad[0]*180/M_PI << std::endl;
    // std::cout << "qdInfo.azAoa_deg: " << qdInfo.azAoa_rad[0]*180/M_PI << std::endl;
    // std::cout << "qdInfo.elAod_deg: " << qdInfo.elAod_rad[0]*180/M_PI << std::endl;
    // std::cout << "qdInfo.elAoa_deg: " << qdInfo.elAoa_rad[0]*180/M_PI << std::endl;
    // std::cout << "qdInfo.elAoa_deg: " << qdInfo.elAoa_rad*180/M_PI << std::endl;
    // std::cout << "qdInfo.azAod_deg: " << qdInfo.azAod_rad*180/M_PI << std::endl;
    // std::cout << "qdInfo.elAod_deg: " << qdInfo.elAod_rad*180/M_PI << std::endl;
    ...
    // Print channel matrix in organized format
    // std::cout << "\n=== Channel Matrix Details ===\n";
    // for (size_t cluster = 0; cluster < H.GetNumPages(); ++cluster)
    // {
    //     std::cout << "\nCluster " << cluster << ":\n";
    //     for (size_t rx = 0; rx < H.GetNumRows(); ++rx)
    //     {
    //         std::cout << "Rx " << rx << ": ";
    //         for (size_t tx = 0; tx < H.GetNumCols(); ++tx)
    //         {
    //             std::cout << std::fixed << std::setprecision(6) << H(rx, tx, cluster) << "\t";
    //         }
    //         std::cout << "\n";
    //     }
    // }
    ...
    // // Print delays for each cluster
    // std::cout << "\n=== Cluster Delays ===\n";
    // for (uint8_t cIndex = 0; cIndex < channelParams->m_delay.size(); ++cIndex)
    // {
    //     std::cout << "Cluster " << static_cast<int>(cIndex) << " delay: " << std::scientific
    //               << std::setprecision(3) << channelParams->m_delay[cIndex] << " seconds\n";
    // }
    // std::cout << "===================\n\n";
